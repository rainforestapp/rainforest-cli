...
func TestShuffleFlags(t *testing.T) {
	var testCases = []struct {
		...
		{
			testArgs: []string{"./rainforest", "run", "--tags", "tag,bag", "--token", "foobar", "--site", "123", "--run-group", "14"},
			want:     []string{"./rainforest", "--token", "foobar", "run", "--tags", "tag,bag", "--site", "123", "--run-group", "14"},
		},
		{
