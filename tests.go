...
// Returns the browsers array with a single browser name, or nil if not specified.
func parseAndValidateBrowser(c cliContext) ([]string, error) {
	...
	// Validate that the platform name only contains valid characters
	validPlatform := regexp.MustCompile(`^[a-z0-9_]+$`)
	if !validPlatform.MatchString(platform) {
		return nil, cli.NewExitError("Invalid platform name. Platform must only contain lowercase letters, numbers, and underscores", 1)
	}

	return []string{platform}, nil
}

// generateAITest generates a new test using AI based on a prompt
func generateAITest(c cliContext, api rfAPI) error {
	prompt := c.Args().First()
	if prompt == "" {
		return cli.NewExitError("Please provide a prompt describing the test you want to generate", 1)
	}

	title := c.String("title")
	startURI := c.String("start-uri")
	fullURL := c.String("url")

	// Require either start_uri or full_url, but not both
	if startURI == "" && fullURL == "" {
		return cli.NewExitError("Please provide either --start-uri or --url for the test", 1)
	}
	if startURI != "" && fullURL != "" {
		return cli.NewExitError("Please provide either --start-uri or --url, not both", 1)
	}

	// Parse and validate browser
	browsers, err := parseAndValidateBrowser(c)
	if err != nil {
		return err
	}

	request := &rainforest.AITestRequest{
		Title:    title,
		Type:     "test",
		Prompt:   prompt,
		Browsers: browsers,
	...
	if startURI != "" {
		request.StartURI = startURI
	}
	if fullURL != "" {
		request.FullURL = fullURL
	}

	if envID := c.Int("environment-id"); envID > 0 {
		request.EnvironmentID = envID
	}

	creds := c.String("credentials")
	loginSnippetID := c.Int("login-snippet-id")

	// Validate mutually exclusive login parameters
	if creds != "" && loginSnippetID > 0 {
		return cli.NewExitError("Cannot specify both --credentials and --login-snippet-id. Choose one login method", 1)
	}

	if creds != "" {
		request.PromptCredentials = creds
	}
	if loginSnippetID > 0 {
		request.LoginSnippetID = loginSnippetID
	}

	log.Printf("Generating test with AI...")

	response, err := api.CreateTestWithAI(request)
	if err != nil {
		return cli.NewExitError(err.Error(), 1)
	}

	log.Printf("Test #%d created: %s", response.TestID, response.Title)
	log.Printf("Note: The test is still being generated by AI and may take a few moments to complete.")
...
type rfAPI interface {
	...
	ClientToken() string
	CreateTestWithAI(*rainforest.AITestRequest) (*rainforest.AITestResponse, error)
	branchAPI
}
...
func downloadTests(c cliContext, client rfAPI) error {
	...
	for i := 0; i < len(testInfos); i++ {
		...
		case err = <-errorsChan:
			return cli.NewExitError(err.Error(), 1)
		case test := <-testChan:
			if err := downloadTestAsRFML(test, *testIDCollection, absTestDirectory, c.Bool("flatten-steps")); err != nil {
				return cli.NewExitError(err.Error(), 1)
			}
		}
	}
...
// downloadTestAsRFML downloads a test, prepares it as RFML, and writes it to a file
func downloadTestAsRFML(test *rainforest.RFTest, testIDCollection rainforest.TestIDCollection, absTestDirectory string, flattenSteps bool) error {
	err := test.PrepareToWriteAsRFML(testIDCollection, flattenSteps)
	if err != nil {
		return err
	}
	...
	if err := writer.WriteRFMLTest(test); err != nil {
		return err
	}
